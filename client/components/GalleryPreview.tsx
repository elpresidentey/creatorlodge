      ...
      <div className="lounge-container section-pad">
        ...
        <div className="grid gap-5 lg:grid-cols-2 lg:gap-6">
          <Reveal>
            <Link
              to="/gallery"
              className="group card-lift pressable relative block min-h-[420px] overflow-hidden lg:min-h-full"
            >
              <img
                src={now.image}
                alt={now.title}
              ...
              <div className="overlay-text absolute inset-x-0 bottom-0 p-6 md:p-8">
                <p className="photo-meta">{now.edition} · {now.venue}</p>
                <h3 className="mt-2 font-display text-[20px] font-medium leading-tight text-ivory line-clamp-2 min-h-[50px] md:text-[24px]">{now.title}</h3>
                <p className="mt-2 max-w-md photo-desc">{now.blurb}</p>
          ...
          <div className="grid gap-5">
            {next.map((e, i) => (
              <Reveal key={e.slug} delay={i * 110}>
                <Link
                  to="/gallery"
                  className="group card-lift pressable relative block min-h-[200px] overflow-hidden"
                >
                  <img
                    src={e.image}
                    alt={e.title}
