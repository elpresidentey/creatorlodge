      ...
      <div className="lounge-container section-pad">
        <Reveal className="section-head">
          <div className="section-head-copy">
            <p className="eyebrow eyebrow-rule text-[#7D6333]">Private Spaces</p>
            <h2 className="section-title text-[#171410]">Work the way <span className="italic text-[#7D6333]">you wish.</span></h2>
          </div>
          <Link to="/spaces" className="section-link pressable text-[#7D6333] hover:text-[#171410]">Browse all spaces →</Link>
        </Reveal>
          ...
          {spaces.filter(s=>featured.includes(s.id)).map((s, i)=>(
            ...
            <Link to={`/book?space=${s.id}&outlet=${outlets[0].slug}`} className="group card-lift pressable relative block aspect-[3/4] overflow-hidden">
              ...
              <div className="overlay-text absolute inset-x-0 bottom-0 p-5">
                ...
                <p className="mt-1.5 line-clamp-2 min-h-[36px] text-[13px] leading-relaxed text-white/80">{s.desc}</p>
                <p className="mt-3 border-t border-white/20 pt-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-gold-soft">{s.capacity}</p>
              </div>
            </Link>
            </Reveal>
          ))}
